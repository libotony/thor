...
// newJustifier creates a new justifier for the block described by sum.
//
// sum.Header may not yet be in the repo (during bft.Select); sum carries
// the conflicts count so we can still address its post-housekeep state.
//
// Two different state snapshots are used:
//   - qualitySum (checkpoint - 1): last block of the previous round; source of
//     the previous quality and the PoA threshold (max block proposers).
//   - thresholdSum (checkpoint): post-housekeep state of the current epoch's
//     first block; source of PoS total weight. For the checkpoint block itself
//     the checkpoint is not yet in repo, so we construct the trie root from
//     sum.Header.StateRoot() and sum.Conflicts directly.
func (engine *Engine) newJustifier(sum *chain.BlockSummary) (*justifier, error) {
	...
	// For deciding posActive and threshold we need the post-housekeep state of
	// the checkpoint block. The checkpoint block itself is the one described by
	// sum when blockNum == checkpoint; otherwise fetch it from the repo.
	thresholdSum := sum
	...
	// PoA threshold: max block proposers does not depend on housekeep, keep
	// reading it from the previous round's last block (qualitySum).
