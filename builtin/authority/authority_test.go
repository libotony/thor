...
func TestAuthority(t *testing.T) {
	...
	tests := []struct {
		...
		{M(aut.Candidates(big.NewInt(10), thor.InitialGasLimit)), M(
			[]*Candidate{{p1, p1, thor.Bytes32{}, true}, {p2, p2, thor.Bytes32{}, true}, {p3, p3, thor.Bytes32{}, true}}, nil,
		)},
		{M(aut.Candidates(big.NewInt(20), thor.InitialGasLimit)), M(
			[]*Candidate{{p2, p2, thor.Bytes32{}, true}, {p3, p3, thor.Bytes32{}, true}}, nil,
		)},
		{M(aut.Candidates(big.NewInt(30), thor.InitialGasLimit)), M(
		...
		{M(aut.Candidates(&big.Int{}, thor.InitialGasLimit)), M(
			[]*Candidate{{p2, p2, thor.Bytes32{}, true}, {p3, p3, thor.Bytes32{}, true}}, nil,
		)},
	}
