...
func TestEventReader_Read(t *testing.T) {
	...
	er = newEventReader(thorChain.Repo(), genesisBlk.Header().ID(), &dto.SubscriptionEventFilter{})
	...
	var eventMessages []*dto.EventMessage
	for _, event := range events {
		if msg, ok := event.(*dto.EventMessage); ok {
